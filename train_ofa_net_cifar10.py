...
from ofa.cifar10_classification.elastic_nn.networks import OFAMobileNetV3
from ofa.cifar10_classification.run_manager import DistributedCifar10RunConfig
from ofa.cifar10_classification.networks import MobileNetV3Large
...
if __name__ == "__main__":
    ...
    if args.task == "kernel":
        ...
        if args.phase == 1:
            args.ofa_checkpoint_path = "exp/normal2kernel"
        else:
            args.ofa_checkpoint_path = "exp/kernel2kernel_depth/phase1"
        with open(
            os.path.join(args.ofa_checkpoint_path, "checkpoint/latest.txt"), "r"
        ) as file:
            args.ofa_checkpoint_path = file.read().rstrip()
        ...
        if args.phase == 1:
            args.ofa_checkpoint_path = "exp/kernel2kernel_depth/phase2"
        else:
            args.ofa_checkpoint_path = "exp/kernel_depth2kernel_depth_width/phase1"
        with open(
            os.path.join(args.ofa_checkpoint_path, "checkpoint/latest.txt"), "r"
        ) as file:
            args.ofa_checkpoint_path = file.read().rstrip()
